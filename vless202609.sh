...
do_install_server() {
    ...
    case "$protocol" in
        vless)
            ...
            # 兼容新旧 Xray 版本输出格式：
            # 旧版: PrivateKey: xxx / Password: xxx
            # 新版(26.x+): PrivateKey: xxx / Password (PublicKey): xxx / Hash32: xxx
            local privkey=$(echo "$keys" | grep -iE "^Private ?key" | awk '{print $NF}')
            local pubkey=$(echo "$keys" | grep -iE "^Public ?key|^Password" | awk -F': ' '{print $NF}')
        ...
        vless-xhttp)
            ...
            if [[ "$xhttp_mode" == "reality" ]]; then
                ...
                local keys=$(xray x25519 2>/dev/null)
                [[ -z "$keys" ]] && { _err "密钥生成失败"; _pause; return 1; }
                # 兼容新旧 Xray 版本输出格式（新版 26.x+ 的公钥行为 "Password (PublicKey):"）
                local privkey=$(echo "$keys" | grep -iE "^Private ?key" | awk '{print $NF}')
                local pubkey=$(echo "$keys" | grep -iE "^Public ?key|^Password" | awk -F': ' '{print $NF}')
                [[ -z "$privkey" || -z "$pubkey" ]] && { _err "密钥提取失败"; _pause; return 1; }
